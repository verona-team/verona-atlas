...
export async function POST(request: NextRequest) {
  ...
  const { projectId, posthogApiKey, posthogProjectId } = parsed.data
  ...
  const validation = await validatePosthogCredentials(posthogApiKey, posthogProjectId)
  if (!validation.ok) {
  ...
  const config: Json = {
    ...
    api_host: (process.env.POSTHOG_API_HOST || 'https://us.posthog.com').replace(
      /\/$/,
      ''
    ),
  }
