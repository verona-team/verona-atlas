...
export default function TemplatesPage() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      {templates.length === 0 ? (
        <p className="text-sm opacity-30 py-4">No templates yet.</p>
      ) : (
            ...
            <div key={t.id} className="py-3">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  {t.source === 'ai_generated' && <span className="text-xs opacity-30 ml-2">AI</span>}
                  {t.description && <p className="text-xs opacity-40 mt-0.5">{t.description}</p>}
                </div>
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs opacity-50">{editingTemplate ? 'Update details and steps.' : 'Define steps for this test.'}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 text-sm">
            <div>
              <label className="block text-xs opacity-40 mb-1">Name</label>
              <input value={formName} onChange={e => setFormName(e.target.value)} placeholder="Login Flow Test" className="w-full border-b bg-transparent py-1 outline-none placeholder:opacity-30" />
            </div>
            <div>
              <label className="block text-xs opacity-40 mb-1">Description</label>
              <textarea value={formDescription} onChange={e => setFormDescription(e.target.value)} placeholder="What does this test verify?" rows={2} className="w-full border-b bg-transparent py-1 outline-none resize-none placeholder:opacity-30" />
            </div>
            <div className="space-y-3 pt-2">
              <span className="text-xs opacity-40">Steps</span>
                ...
                <div key={i} className="border p-3 space-y-2">
                  ...
                  <div className="grid gap-2 sm:grid-cols-[120px_1fr]">
                    <Select value={step.type} onValueChange={v => updateStep(i, { type: v as StepType })}>
                      ...
                      <SelectContent>{STEP_TYPES.map(t => <SelectItem key={t} value={t} className="text-xs">{t}</SelectItem>)}</SelectContent>
                    </Select>
                    <input value={step.instruction} onChange={e => updateStep(i, { instruction: e.target.value })} placeholder="Instruction" className="w-full border-b bg-transparent py-1 text-xs outline-none placeholder:opacity-30" />
                  </div>
                  {step.type === 'navigate' && <input value={step.url ?? ''} onChange={e => updateStep(i, { url: e.target.value })} placeholder="URL" className="w-full border-b bg-transparent py-1 text-xs outline-none placeholder:opacity-30" />}
                  {step.type === 'assertion' && <input value={step.expected ?? ''} onChange={e => updateStep(i, { expected: e.target.value })} placeholder="Expected" className="w-full border-b bg-transparent py-1 text-xs outline-none placeholder:opacity-30" />}
                </div>
              ))}
              <button onClick={addStep} className="text-xs underline opacity-50 hover:opacity-100">+ Add step</button>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditDialogOpen(false)} className="text-xs">Cancel</Button>
            <Button onClick={handleSave} disabled={saving || !formName.trim() || formSteps.length === 0} className="text-xs">
      ...
      <Dialog open={reviewDialogOpen} onOpenChange={setReviewDialogOpen}>
        <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs opacity-50">Click to select, then save.</DialogDescription>
          </DialogHeader>
          <div className="divide-y text-sm">
            {generatedTemplates.map((gt, idx) => (
              <div key={idx} className={`py-3 cursor-pointer ${acceptedIndices.has(idx) ? 'opacity-100' : 'opacity-50'}`} onClick={() => toggleAccepted(idx)}>
                <div className="flex items-center gap-2">
                  <span>{acceptedIndices.has(idx) ? '☑' : '☐'}</span>
                  <span>{gt.name}</span>
                </div>
                <p className="text-xs opacity-40 mt-0.5">{gt.description}</p>
                <div className="mt-1 space-y-0.5">
                  {gt.steps.map((s, si) => (
                    <div key={si} className="text-xs flex gap-2"><span className="opacity-30">{s.type}</span><span className="opacity-50">{s.instruction}</span></div>
                  ))}
                </div>
              </div>
            ))}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setReviewDialogOpen(false)} className="text-xs">Dismiss</Button>
            <Button onClick={handleSaveGenerated} disabled={savingGenerated || acceptedIndices.size === 0} className="text-xs">
