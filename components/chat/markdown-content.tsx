...
const components: Components = {
  ...
  h1: ({ children }) => (
    <h1 className="text-lg font-semibold mt-5 mb-2 first:mt-0 tracking-tight">{children}</h1>
  ),
  h2: ({ children }) => (
    <h2 className="text-base font-semibold mt-4 mb-2 first:mt-0 tracking-tight">{children}</h2>
  ),
  h3: ({ children }) => (
    <h3 className="text-sm font-semibold mt-3 mb-1.5 first:mt-0 tracking-tight">{children}</h3>
  ),
  h4: ({ children }) => (
  ...
  code: ({ className, children, ...props }) => {
    ...
    if (isBlock) {
      ...
      return (
        <div className="my-3 rounded-lg border border-border overflow-hidden bg-muted">
          {lang && (
            <div className="px-3 py-1 border-b border-border text-[10px] uppercase tracking-wide text-muted-foreground font-mono">
              {lang}
            </div>
          )}
